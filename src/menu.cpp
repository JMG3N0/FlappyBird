...
namespace flappy {
	...
	void DrawMenu(button& title, button& start, button& credits, button& quit)
	{
		...
		if (PointRecColition(mousePosition, start.size, start.position) == true)
		{
			DrawText("Play", static_cast<int>(start.position.x + 30.0f), static_cast<int>(start.position.y), static_cast<int>(start.size.y), RED);
		}
		else if (PointRecColition(mousePosition, start.size, start.position) == false)
		{
		...
		if (PointRecColition(mousePosition, credits.size, credits.position) == true)
		{
			DrawText("Credits", static_cast<int>(credits.position.x), static_cast<int>(credits.position.y), static_cast<int>(credits.size.y), RED);
		}
		else if (PointRecColition(mousePosition, credits.size, credits.position) == false)
		{
		...
		if (PointRecColition(mousePosition, quit.size, quit.position) == true)
		{
			DrawText("Exit", static_cast<int>(quit.position.x + 30.0f), static_cast<int>(quit.position.y), static_cast<int>(quit.size.y), RED);
		}
		else if (PointRecColition(mousePosition, quit.size, quit.position) == false)
		{
